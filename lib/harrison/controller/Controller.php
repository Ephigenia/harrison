...
class Controller extends \ephFrame\core\Controller
{
	...
	{
		return $this->entityManager()->getRepository('harrison\model\\'.$this->name);
	}
	
	protected function attachSearchForm()
	{
		$this->view->SearchForm = new \harrison\form\Search();
		...
		if ($this->view->SearchForm['q']->data) {
			$this->response = new Response(
				HTTPStatusCode::TEMPORARY_REDIRECT, new Header(array(
					'Location' => Router::getInstance()->search(array(
						'q' => $this->view->SearchForm['q']->data,
					))
				))
			);
			return true;
	...
	{
		$this->view = new \harrison\view\ThemeView();
