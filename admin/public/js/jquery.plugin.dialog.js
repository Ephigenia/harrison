...
var DialogWindow = new JS.Class({
	...
	draw: function() {
		if (this.options.title) {
			...
			this.titleBar.append('<a class="closeBtn" href="javascript:void(0);">X</a>');
...
var ModalWindow = new JS.Class(MessageWindow, {
	...
	drawModalOptions: function() {
		...
		for(index in this.modalOptions) {
			var modalOption = $('<a href="javascript:void(0);" class="btn">' + this.modalOptions[index].label + '</a>');
...
var PromptWindow = new JS.Class(MessageWindow, {
	...
	drawPrompt: function() {
		...
		var submit = $('<a href="javascript:void(0);" class="btn">OK</a>');
